<template>
  <div class="company_position_recruiting">
    <div class="position_recruiting_title" style="position:relative">
      <h3><span class="colorText">{{jobDtos.length}}</span>个已下线职位</h3>
      <p>简历处理率提升至70%以上，可增加一次只为刷新次数</p>
      ...
      <ul class="table_inner">
        <li v-for="list in jobDtos">
          ...
          <span class="recruiting_operation">
            <a @click="lineOn(list.id)">上线</a>
            <a @click="delJobs(list.id)">删除</a>
          </span>
        </li>
      </ul>
    </div>
    <div class="position_recruiting_foot">
      <div class="position_recruiting_foot_left">
        ...
        <span>全选</span>
        <a @click="lineOn()">上线</a>
        <a @click="delJobs()">删除</a>
      </div>
      <div id="pagebar" class="pagination" v-if="pageable.WhetherNotShown"
           style="float: right;width: 735px; margin-top: 0">
        <ul style="float: right">
          ...
          <li v-for="list in pageable.pageNumber" :class="{'active':list.active}" @click="TurnThePage(list.Number)"><a>{{list.Number}}</a>
          </li>
          <li :class="{'disabled':pageable.pageNo>=pageable.maxPage}" @click="TurnThePage(pageable.pageNo+1)"><a>下一页</a>
          </li>
          <li :class="{'disabled':pageable.pageNo>=pageable.maxPage}" @click="TurnThePage(pageable.maxPage)"><a>尾页</a>
          </li>
        </ul>
      </div>
    </div>
...
<script>
  export default {
    ...
    methods: {
      initdata: function () {
        this.postData("/api/jobManager/jobSearch", {
          companyId:this.UploadData.companyId,
          jobName: this.UploadData.jobName,
          state: 2,
      ...
      lineOn(listId){
        ...
        this.postData("/api/jobManager/lineOn",{ids:ids2,days:"14"},{}).then((res)=>{
          if(res.data.code==0){
            this.ids=[];
            this.initdata();
          }else{
      ...
      delJobs (listId){
        ...
        this.postData("/api/jobManager/delJobs",{ids:ids2},{}).then((res)=>{
          if(res.data.code==0){
            this.ids=[];
            this.initdata();
          }else{
