    ...
    <div class="company_register_box">
      ...
      <div class="phone_register_box">
        ...
        <div class="register_box_2">
          <p>已有账号？直接 <router-link to="/companyhome">登录</router-link></p>
          <!--二维码-->
  ...
  export default {
    ...
    methods:{//方法
      ...
      JumpPerfect_information:function(){
        let vue = this
        this.user.identifier=this.phone;
      ...
      AllFillIn:function(){
        let _this=this;
        setTimeout(function(){
          if(_this.phone!=""&&_this.password!=""&&_this.captcha!=""&& _this.notToApprove==true){
            _this.registrationIsAvailable=true;
          }else{
            _this.registrationIsAvailable=false;
          }
        },500)
      },
      //注册
      registered:function(){
        if(this.registrationIsAvailable){
          this.postData2('/api/member/add',{captcha: this.captcha,mobile:this.phone,password:this.password,type:"2"}).then((res) => {
            console.log(res.data);
            if(res.data.code==0){
